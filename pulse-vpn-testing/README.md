[verification_reports_ru.md](https://github.com/user-attachments/files/22363869/verification_reports_ru.md)[Uploading test_plan.md…]()
[test_plan_ru.md](https://github.com/user-attachments/files/22363857/test_plan_ru.md)
...
| :--- | :--- | :--- | :--- | :--- |
| **TC-POOL-01** | Пул ключей | Проверка отключения авто-пополнения для одного сервера | 1. Перейтив "Settings" -> "Key Pooling". <br> 2. Выключить переключатель для сервера "Germany". <br> 3. Вручную запустить цикл пополнения пула ключей. | `AutoKeyPoolManager` не должен назначать новые ключи на сервер "Germany". Количество ключей для этого сервера в пуле должно остаться неизменным. |
| **TC-METRICS-01** | Метрики | Проверка корректного отображения метрик для обновленного Outline-сервера (JSON) | 1. Перейтив на вкладку "Servers". <br> 2. Наблюдать за карточкой Outline-сервера, на котором подтвержден запуск нового скрипта метрик на основе JSON. | Карточка сервера должна отображать "Live Metrics" с корректными значениями для CPU, Memory, Load и т.д. В консоли не должно быть ошибок. |
| **TC-METRICS-02** | Метрики | Проверка корректного отображения [bug_reports_ru.md](https://github.com/user-attachments/files/22363864/bug_reports_ru.md)
метрик для обновленного Shadowsocks-сервера (JSON) | 1. Перейтив на вкладку "Servers". <br> 2. Наблюдать за карточкой Shadowsocks-сервера, на котором подтвержден запуск нового скрипта метрик на основе JSON. | Карточка сервера должна отображать "Live Metrics" с корректными значениями для CPU, Memory, Load и т.д. В консоли не должно быть ошибок. |
| **TC-METRICS-03** | Метрики | Проверка корректной обработки серверов со старым скриптом метрик (текстовым) | 1. Перейтив на вкладку "Servers". <br> 2. Наблюдать за карточкой сервера, на котором заведомо работает старый скрипт метрик. | Приложение не должно падать. Карточка сервера должна отображать ошибку или резервное состояние (например, "Метрики недоступны"). В консоль должна быть записана ошибка декодирования JSON. |
| **TC-UI-01** | UI | Проверка динамического окрашивания значений метрик | 1. Перейтив на вкладку "Servers". <br> 2. Наблюдать за сервером с высокой нагрузкой (CPU > 90%). | Числовое значение метрики CPU должно быть окрашено в красный цвет. |
| **TC-UI-02** | UI | Проверка отсутствия переноса текста в метриках | 1. Перейтив на вкладку "Servers". <br> 2. Наблюдать за секцией "Live Metrics". | Заголовки "Conns" и "Docker" должны быть полностью видны на одной строке. |

---[Uploading bug_reports_ru.md…]()
